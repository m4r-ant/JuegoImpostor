...
export function JoinRoom({ onBack }: Props) {
    ...
    <main className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-3xl font-bold text-lime-400">Unirse a Partida</h2>
          <p className="text-gray-400 text-sm">Necesitas el código de la sala</p>
        </div>
        ...
        <form onSubmit={handleJoin} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-mono text-gray-300 mb-2">Código de la sala</label>
            <input
              type="text"
              value={code}
              onChange={(e) => setCode(e.target.value.toUpperCase())}
              placeholder="Ej: ABC123"
              className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-lime-400 focus:outline-none transition-colors font-mono text-center text-lg tracking-widest"
              maxLength={6}
              disabled={loading}
            />
          </div>
        ...
        <div className="text-gray-600 text-xs text-center">El anfitrión comparte el código contigo</div>
      </div>
    </main>
  )
}
